...
export = {
  init({ baseUrl, rootPath, hiddenPath, s3Options, ...legacyS3Options }: InitOptions) {
    ...
    const hiddenPrefix = hiddenPath ? `${hiddenPath.replace(/\/+$/, '')}/` : ''

    const filePrefix = hiddenPrefix + rootPath ? `${rootPath.replace(/\/+$/, '')}/` : '';
        ...
        const onUploaded = (err: Error, data: AWS.S3.ManagedUpload.SendData) => {
          ...
          if (baseUrl) {
            ...
            if (hiddenPrefix === '') {
              ...
              file.url = `${baseUrl}/${fileKey.replace(hiddenPrefix, '')}`;
            }
          } else {
